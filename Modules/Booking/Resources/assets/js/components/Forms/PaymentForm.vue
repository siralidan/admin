...
const calculatePercentAmount = (percent) => {
  ...
  const percentAmount = ((data.booking_amount+data.product_amount) * percent) / 100;

  return percentAmount
}
const subtotal = computed(() => {
  return data.booking_amount + data.product_amount;
});
...
const taxAmount = computed(() => {
  ...
  for (const tax of data.taxes) {
    ...
    if (tax.type === 'percent') {
      ...
      totalTaxAmount += ((data.booking_amount+data.product_amount) *tax.percent) / 100;
...
const finalAmount = computed(() => {
  ...
  submitPayment()
  const val = window.currencyFormat((
